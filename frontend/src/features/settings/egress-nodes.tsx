...
const emptyInput: EgressNodeInput = { name: "", scope: "grok_build", enabled: true, forceNoCooldown: false, proxyPool: false, proxyURL: "", userAgent: "", cloudflareCookies: "" };
...
export function EgressNodes({ clearanceMode }: { clearanceMode: "manual" | "flaresolverr" }) {
  ...
  function openEdit(node: EgressNodeDTO) {
    setForm({ name: node.name, scope: node.scope, enabled: node.enabled, forceNoCooldown: node.forceNoCooldown, proxyPool: node.proxyPool, userAgent: node.scope === "grok_build" ? "" : node.userAgent, proxyURL: "", cloudflareCookies: "" });
    setEditing(node);
  }
