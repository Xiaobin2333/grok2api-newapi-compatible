        ...
        <Tabs defaultValue="build" className="flex flex-col gap-7 lg:flex-row lg:items-start">
          ...
          <SettingsSection title={t("settings.web.title")}>
            <div className="space-y-0">
                ...
                <Controller control={form.control} name="providerWeb.statsigMode" render={({ field }) => (
                  <Tabs value={field.value} onValueChange={field.onChange}>
                    <TabsList id="web-statsig-mode" className="grid w-full grid-cols-3 bg-muted/55">
                      <TabsTrigger value="local" className="font-normal">{t("settings.web.statsigLocal")}</TabsTrigger>
              ...
              <SettingsField controlId="web-basic-image-edit" label={t("settings.web.enableBasicImageEditViaChat")} description={t("settings.web.enableBasicImageEditViaChatHelp")}><Controller control={form.control} name="providerWeb.enableBasicImageEditViaChat" render={({ field }) => <div className="flex h-8 items-center"><Switch id="web-basic-image-edit" checked={field.value} onCheckedChange={field.onChange} /></div>} /></SettingsField>
            </div>
          </SettingsSection>
          </SettingsPane>
          ...
          <SettingsSection title={t("settings.routing.title")}>
            <div className="space-y-0">
              ...
              <SettingsField controlId="routing-max-attempts" label={t("settings.routing.maxAttempts")} description={t("settings.routing.maxAttemptsHelp")} error={form.formState.errors.routing?.maxAttempts?.message}><Input id="routing-max-attempts" type="number" min={0} {...form.register("routing.maxAttempts", { valueAsNumber: true })} /></SettingsField>
              <SettingsField controlId="routing-free-account-max-concurrent" label={t("settings.routing.freeAccountMaxConcurrent", { defaultValue: "Free/Basic account concurrency limit" })} description={t("settings.routing.freeAccountMaxConcurrentHelp", { defaultValue: "Maximum concurrent requests for each confirmed Build Free or Web Basic account. 0 uses the account's own concurrency setting." })} error={form.formState.errors.routing?.freeAccountMaxConcurrent?.message}><Input id="routing-free-account-max-concurrent" type="number" min={0} max={256} {...form.register("routing.freeAccountMaxConcurrent", { valueAsNumber: true })} /></SettingsField>
              <SettingsField controlId="routing-video-premium-account-attempts" label={t("settings.routing.videoPremiumAccountAttempts", { defaultValue: "Super/Heavy video attempts per account" })} description={t("settings.routing.videoPremiumAccountAttemptsHelp", { defaultValue: "Total attempts on one Super/Heavy account for retryable Grok Web video 403 responses; regular accounts are still attempted once." })} error={form.formState.errors.routing?.videoPremiumAccountAttempts?.message}><Input id="routing-video-premium-account-attempts" type="number" min={1} {...form.register("routing.videoPremiumAccountAttempts", { valueAsNumber: true })} /></SettingsField>
