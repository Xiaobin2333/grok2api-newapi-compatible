...
- **Grok 图片自动路由**：调用方可统一请求 `grok-imagine-image`；存在参考图时，在能力检查、账号选择、计费和审计创建之前规范为 `grok-imagine-image-edit`。
- **OpenAI 图片编辑兼容**：`/v1/images/edits` 同时接受 JSON URL 和 OpenAI SDK 的 `multipart/form-data` 上传，支持 `image`、`image[]`、`images`、`images[]`、`image_urls`、`image_urls[]`。multipart 文本值还可使用 URL 字符串、`{ "url": "..." }` 对象和混合 JSON 数组；URL、`b64_json` 与 SSE 响应继续沿用现有 Images 协议。
- **Free/Basic Chat 模拟编辑**：管理后台开关允许 Basic Web 账号通过 `grok-chat-fast` 图片附件模拟 `grok-imagine-image-edit`。Adapter 使用严格的图生图保持契约、隔离多参考图编号语义、关闭 side-by-side 变体，并在调用方省略比例或使用 `auto` 时继承第一张参考图的真实比例。
- **参考图构图处理**：显式目标比例会按本地可读图片的真实宽高选择最接近的参考图置前，并同步重写 prompt 中的 `image 1` / `图片1` 编号，使角色、场景、道具和画风语义保持一致。
- **账号池可靠性**：`maxAttempts: 0` 对每个符合能力的账号最多尝试一轮；正数尝试次数不限制输入上限。Free/Basic 每账号并发和 Super/Heavy 视频单账号尝试次数可独立配置，Lite 多图任务可跨账号续跑。
- **出口代理控制**：单个代理可强制不进入冷却；共享代理池的单次连接故障不会污染节点整体健康，包含 `{account}` 的账号绑定代理会自动使用代理池策略。
- **Grok Web anti-bot 回退**：保留远端或手动 Statsig 协议；远端签名服务直连失败时复用当前 Web 出口重试。`403/code 7` 会失效当前账号的浏览器会话并重租出口，固定次数模式先重试同账号一次再切号，`maxAttempts: 0` 仍只轮询所有账号一轮。
- **媒体持久化与诊断**：按需持久化图片/视频输入输出，失败尝试记录脱敏后的上游诊断，异步视频任务保留账号、出口、计费和审计上下文。
- **NewAPI 与管理端兼容**：图片/视频请求类型、模型发现、计费、审计、运行设置、API 文档和管理视图均覆盖二开路由与可靠性设置。
...
| `grok-imagine-image-edit` | 图片编辑 | Basic 开启 Chat 模拟；Super 原生 |
