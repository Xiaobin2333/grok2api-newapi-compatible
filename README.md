...
- **Grok Web anti-bot failover**: remote and manual Statsig flows remain intact, while direct remote-signer failures retry through the active Web egress. A `403/code 7` invalidates only the rejected account's browser session and reacquires egress; bounded mode retries that account once before rotating, and `maxAttempts: 0` still visits every account only once.
